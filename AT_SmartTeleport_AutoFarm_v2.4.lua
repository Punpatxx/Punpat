-- ==========================================
-- ⚡ AT Hub - Smart Teleport & Auto Farm Engine v2.4
-- 👑 God-Tier Ultimate Edition (Low HP Escape, UI Minimize & Auto Reconnect)
...
local safeKey = "AT_SmartEngine_v24"
...
TrackConnection(CoreGui.ChildAdded:Connect(function(child)
    if child.Name == "ErrorPrompt" and Config.AutoReconnectOn then
        task.spawn(function()
            while true do
    ...
    pcall(function()
        if state then
            ...
            for _, v in ipairs(Lighting:GetChildren()) do
                if v:IsA("PostEffect") or v:IsA("BlurEffect") or v:IsA("SunRaysEffect") or v:IsA("ColorCorrectionEffect") or v:IsA("Atmosphere") then
                    v.Enabled = false
                end
            end
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    MainFrame.Size = isMinimized and UDim2.new(0, 380, 0, 32) or UDim2.new(0, 380, 0, 430)
...
MakeSlider(pageMob, "ความเร็วบินหามอน", 10, 200, 40, function(v) Config.MobSpeed = v end)
...
local lowHPEscapeToggle = Instance.new("TextButton")
lowHPEscapeToggle.Size = UDim2.new(0.95, 0, 0, 32)
lowHPEscapeToggle.BackgroundColor3 = Color3.fromRGB(24, 24, 36)
lowHPEscapeToggle.TextColor3 = Color3.fromRGB(240, 240, 250)
lowHPEscapeToggle.Text = "[OFF] หนีตายเลือดน้อยอัตโนมัติ"
lowHPEscapeToggle.Font = Enum.Font.GothamBold
lowHPEscapeToggle.TextSize = 10
lowHPEscapeToggle.Parent = pageMove
Instance.new("UICorner", lowHPEscapeToggle).CornerRadius = UDim.new(0, 8)
lowHPEscapeToggle.MouseButton1Click:Connect(function()
    Config.LowHPEscapeOn = not Config.LowHPEscapeOn
    lowHPEscapeToggle.BackgroundColor3 = Config.LowHPEscapeOn and Color3.fromRGB(0, 140, 255) or Color3.fromRGB(24, 24, 36)
    lowHPEscapeToggle.Text = Config.LowHPEscapeOn and "[ON] หนีตายเลือดน้อยอัตโนมัติ" or "[OFF] หนีตายเลือดน้อยอัตโนมัติ"
end)
MakeSlider(pageMove, "เปอร์เซ็นต์เลือดหนีตาย (%)", 10, 60, 25, function(v) Config.LowHPPct = v end)

local autoReconnectToggle = Instance.new("TextButton")
autoReconnectToggle.Size = UDim2.new(0.95, 0, 0, 32)
autoReconnectToggle.BackgroundColor3 = Color3.fromRGB(0, 140, 255)
autoReconnectToggle.TextColor3 = Color3.fromRGB(240, 240, 250)
autoReconnectToggle.Text = "[ON] รีคอนเนกต์อัตโนมัติเมื่อหลุด"
autoReconnectToggle.Font = Enum.Font.GothamBold
autoReconnectToggle.TextSize = 10
autoReconnectToggle.Parent = pageMove
Instance.new("UICorner", autoReconnectToggle).CornerRadius = UDim.new(0, 8)
autoReconnectToggle.MouseButton1Click:Connect(function()
    Config.AutoReconnectOn = not Config.AutoReconnectOn
    autoReconnectToggle.BackgroundColor3 = Config.AutoReconnectOn and Color3.fromRGB(0, 140, 255) or Color3.fromRGB(24, 24, 36)
    autoReconnectToggle.Text = Config.AutoReconnectOn and "[ON] รีคอนเนกต์อัตโนมัติเมื่อหลุด" or "[OFF] รีคอนเนกต์อัตโนมัติเมื่อหลุด"
end)

local fpsBoostToggleBtn = Instance.new("TextButton")
fpsBoostToggleBtn.Size = UDim2.new(0.95, 0, 0, 32)
fpsBoostToggleBtn.BackgroundColor3 = Color3.fromRGB(24, 24, 36)
fpsBoostToggleBtn.TextColor3 = Color3.fromRGB(240, 240, 250)
fpsBoostToggleBtn.Text = "[OFF] เร่งความลื่นขั้นสุด (FPS Boost)"
fpsBoostToggleBtn.Font = Enum.Font.GothamBold
fpsBoostToggleBtn.TextSize = 10
fpsBoostToggleBtn.Parent = pageMove
Instance.new("UICorner", fpsBoostToggleBtn).CornerRadius = UDim.new(0, 8)
fpsBoostToggleBtn.MouseButton1Click:Connect(function()
    Config.FPSBoostOn = not Config.FPSBoostOn
    fpsBoostToggleBtn.BackgroundColor3 = Config.FPSBoostOn and Color3.fromRGB(0, 140, 255) or Color3.fromRGB(24, 24, 36)
    fpsBoostToggleBtn.Text = Config.FPSBoostOn and "[ON] เร่งความลื่นขั้นสุด (FPS Boost)" or "[OFF] เร่งความลื่นขั้นสุด (FPS Boost)"
    ApplyFPSBoost(Config.FPSBoostOn)
end)
    ...
    pcall(function()
        ...
        if hum then
            if Config.AutoAttackOn and Config.SelectedTool then
                ...
                if backpack then targetTool = backpack:FindFirstChild(toolName) end
                if not targetTool then targetTool = player.Character:FindFirstChild(toolName) end
                if targetTool then
                    Config.SelectedTool = targetTool
                    hum:EquipTool(targetTool)
                end
            end
            if Config.FPSBoostOn then ApplyFPSBoost(true) end
        end
    end)
end))
...
TrackConnection(RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        if Config.LowHPEscapeOn and hum then
            local hpPercent = (hum.Health / hum.MaxHealth) * 100
            if hpPercent <= Config.LowHPPct then
        ...
        if Config.ActiveMode == "Player" and Config.TargetPlayer then
            ...
            if IsAlive(tChar) then
                ...
                if tHrp then
                    ...
                    hrp.CFrame = hrp.CFrame:Lerp(targetCF, math.clamp(Config.PlayerSpeed / 100, 0.05, 0.5))
                end
            end
        end
        ...
        if Config.ActiveMode == "AutoMob" then
            ...
            if Config.StickyAutoMobInstance and IsAlive(Config.StickyAutoMobInstance) then
                ...
                if mHrp and (hrp.Position - mHrp.Position).Magnitude <= Config.MobScanRange * 3 then
                    ...
                    hrp.CFrame = hrp.CFrame:Lerp(targetCF, math.clamp(Config.MobSpeed / 100, 0.05, 0.5))
                end
            end
