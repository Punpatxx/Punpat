...
frameStroke.Color = Color3.fromRGB(40, 40, 60)
...
scanToolBtn.MouseButton1Click:Connect(function()
    ...
    local function AddTools(parent)
        ...
        for _, v in ipairs(parent:GetChildren()) do
            if v:IsA("Tool") and not table.find(foundTools, v) then
    ...
    for _, tool in ipairs(foundTools) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            toolConnection = tool.AncestryChanged:Connect(function(_, newParent)
                if not newParent
                    or (newParent ~= player.Character and newParent ~= player:FindFirstChild("Backpack")) then
    ...
    if Config.ProximityAuraOn and Config.SelectedTool and Config.SelectedTool.Parent then
        ...
        for _, v in ipairs(pool) do
            ...
            if tChar and tChar ~= char and IsAlive(tChar) then
                ...
                if p and Config.TeamCheck and p.Team == player.Team then
                    continue
                end
...
if player.Character then
    BackupStats(player.Character:FindFirstChildOfClass("Humanoid"))
end
